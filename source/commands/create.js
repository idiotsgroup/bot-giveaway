...
module.exports = class extends Command {
    ...
    async execute(message) {
        ...
        if (!match) return message.channel.send(`Invalid Command Usage: \`${this.client.config.prefix}create [ايدي الروم او منشن الروم][\'-w <عدد الفائزين\'] <#d #h #m #s|time-in-seconds> <جائزة المسابقة>\``);
        ...
        if (filtered >= 5) return message.channel.send(`خطأ: لكل روم كتابي  اقصى عدد للقيف اوايات 5 قيف اواي فقط.`);
        ...
        if (message.deletable) message.delete({ timeout: 1000, reason: "Giveaway Bot AR" });
